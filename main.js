...
import { Sky } from 'https://cdn.jsdelivr.net/npm/three@0.162.0/examples/jsm/objects/Sky.js';

(() => {
const PLAYER={
eyeHeight:1.65,walkSpeed:3,runSpeed:6,accel:100,airAccel:12,friction:0.92,jumpPower:10,jumpBoost:1.05,
gravity:-40,maxHSpeed:100,coyoteTime:0.25,jumpBuffer:0.2};
...
const sun=new THREE.Vector3(); let timeOfDay=0;
const sunSphere=new THREE.Mesh(new THREE.SphereGeometry(20,16,8),
...
const player={
  ...
  coyoteTimer:0,jumpBufferTimer:0,runCap:PLAYER.runSpeed
};
...
'KeyA':'left','ArrowLeft':'left','KeyD':'right','ArrowRight':'right','ShiftLeft':'run',
'ShiftRight':'run','Space':'jump'};
...
tracker.style.position='absolute'; tracker.style.top='10px'; tracker.style.left='10px';
tracker.style.color='#fff'; tracker.style.background='rgba(0,0,0,0.5)';
tracker.style.padding='6px'; tracker.style.fontFamily='monospace'; tracker.style.whiteSpace='pre';
...
function getCameraDir(){
  ...
  dir.normalize(); dir.applyAxisAngle(new THREE.Vector3(0,1,0),player.yaw);
  return dir;
}

// Explosions
const explosions=[]; const raycaster=new THREE.Raycaster(); const mouse=new THREE.Vector2(0,0);
function spawnExplosionAt(point){
  ...
  sphere.position.copy(point); scene.add(sphere);
  explosions.push({mesh:sphere,time:0,point:point});
  const toPlayer=player.pos.clone().sub(point); const dist=toPlayer.length();
  if(dist<50){ const force=(50-dist)/50*120; toPlayer.normalize(); player.explosionVel.add(toPlayer.multiplyScalar(force)); }
}
...
let lastShot=0; const fireRate=0.12;
function doMuzzleFlash(pos){
  ...
  m.position.copy(pos); scene.add(m); let t=0;
  const interval=setInterval(()=>{
    t+=0.016; m.scale.setScalar(1+t*6); m.material.opacity=Math.max(0,1-t*3);
    if(t>0.2){scene.remove(m); clearInterval(interval);}
  },16);
}
document.addEventListener('mousedown', e=>{
  const now=performance.now()/1000;
  if(now-lastShot<fireRate) return; lastShot=now;
  const barrelWorld=new THREE.Vector3(); barrel.getWorldPosition(barrelWorld);
...
function applyExplosions(dt){for(let i=explosions.length-1;i>=0;i--){const ex=explosions[i];ex.time+=dt; ex.mesh.scale.setScalar(1+ex.time*4); ex.mesh.material.opacity=0.25*(1-ex.time/0.5); if(ex.time>0.5){scene.remove(ex.mesh); explosions.splice(i,1);}}}
...
function animate(){
  ...
  // Day/Night
  timeOfDay+=dt*60; if(timeOfDay>360) timeOfDay-=360;
  const phi=THREE.MathUtils.degToRad(90-(Math.sin(timeOfDay*Math.PI/180)*90));
  const theta=THREE.MathUtils.degToRad(timeOfDay);
  sun.setFromSphericalCoords(1,phi,theta); skyU['sunPosition'].value.copy(sun);
  sunLight.position.copy(sun).multiplyScalar(50); sunSphere.position.copy(sun).multiplyScalar(400);
  const brightness=Math.max(0,sun.y); sunLight.intensity=0.6*brightness; scene.children[0].intensity=0.2+0.5*brightness;
  ...
  const dir=getCameraDir(); let hVel=new THREE.Vector3(player.vel.x,0,player.vel.z);
  let currentSpeed=hVel.length(); const targetSpeed=(keys.run?player.runCap:PLAYER.walkSpeed);

  if(dir.lengthSq()>0){
    if(currentSpeed<1e-4){hVel.copy(dir.clone().multiplyScalar(targetSpeed));currentSpeed=targetSpeed;}
    else{const accel=player.grounded?PLAYER.accel:PLAYER.airAccel; hVel.addScaledVector(dir,accel*dt);
      if(hVel.length()>targetSpeed) hVel.setLength(targetSpeed); currentSpeed=hVel.length(); hVel.copy(dir.clone().multiplyScalar(currentSpeed));}
  }
  if(player.grounded && dir.lengthSq()===0) hVel.multiplyScalar(PLAYER.friction);
  player.vel.x=hVel.x+player.explosionVel.x; player.vel.z=hVel.z+player.explosionVel.z; player.vel.y+=PLAYER.gravity*dt;
  ...
  if(player.jumpBufferTimer>0&&(player.grounded||player.coyoteTimer>0)){
    ...
    player.vel.y=PLAYER.jumpPower-flatten; player.runCap*=PLAYER.jumpBoost;
  }

  player.pos.addScaledVector(player.vel,dt); const wrapLimit=1000;
