(() => {
const PLAYER={
  eyeHeight:1.65, walkSpeed:3, runSpeed:6, accel:100, airAccel:12, friction:0.92,
  jumpPower:10, jumpBoost:1.05, gravity:-40, maxHSpeed:100, coyoteTime:0.25, jumpBuffer:0.2
};
...
const sunLight=new THREE.DirectionalLight(0xffffff,0.6); scene.add(sunLight);
...
document.addEventListener('mousemove', e=>{
  ...
  const sens=0.0022; player.yaw-=e.movementX*sens; player.pitch-=e.movementY*sens;
...
document.addEventListener('mousedown',()=>{
  ...
  spawnExplosionAt(player.pos.clone().add(forward.clone().multiplyScalar(5)));
});
...
function applyExplosions(dt){
  for(let i=explosions.length-1;i>=0;i--){
    ...
    ex.mesh.scale.setScalar(1+ex.time*4); ex.mesh.material.opacity=0.25*(1-ex.time/0.5);
...
function updateSky(dt){
  ...
  const brightness=Math.max(0,sun.y); sunLight.intensity=0.6*brightness; hemiLight.intensity=0.2+0.5*brightness;
}
...
function animate(){
  ...
  if(dir.lengthSq()>0){
    const accel=player.grounded?PLAYER.accel:PLAYER.airAccel; hVel.addScaledVector(dir,accel*dt);
    if(hVel.length()>targetSpeed && currentSpeed<=player.runCap) hVel.setLength(targetSpeed);
    currentSpeed=hVel.length(); hVel.copy(dir.clone().multiplyScalar(currentSpeed));
  } else {if(player.grounded) hVel.multiplyScalar(PLAYER.friction);}
  ...
  applyExplosions(dt); updateCamera(); updateSky(dt);
...
animate();
addEventListener('resize',()=>{camera.aspect=innerWidth/window.innerHeight;camera.updateProjectionMatrix();renderer.setSize(window.innerWidth,window.innerHeight);});
})();
